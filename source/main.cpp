...
public:
	...
	SlgTokenItem(SlgRuleTable::e_SlgTokenType t, SlgRuleTable::e_SlgTokenDesc desc, TokenValueType v, int lineNumber, int colNumber)
	...
	void Show() {
		...
		case SlgRuleTable::e_SlgTokenType::FLOAT:
			std::cout << "[NUMBER(Float)     |" << TypeDescString(m_Desc) << " | Priority: " << m_Priority << " \t| pos( " << m_LineNumber << "," << m_ColNumber << ") | " << GetValue<float>() << "\n";
			break;
...
class SlgStatementCompile {
	...
	SlgTokenizer* m_Tokenizer = nullptr;
	std::vector<SlgTokenItem> m_Tokens;
	...
	std::vector<StatementSegment> m_VarStack{};
	std::array<char, 64> m_Register{  };
	StatementSegment m_imdRegister{};

public:
	inline SlgStatementCompile(SlgTokenizer* tokenizer, SlgErrorMachine* errorMachine) : m_Tokenizer(tokenizer), m_ErrorMachine(errorMachine) {
		m_Tokenizer->PrintTokens();
		m_Tokens = std::move(m_Tokenizer->GetTokens());
...
private:
	...
	//In R1s;
	inline StatementSegment ExprSolver(size_t start_idx, size_t end_idx, int recursionDepth = 0) {

		std::vector<StatementSegment> tlist;
		...
		while (r_idx <= end_idx) {
			...
			else if (bracketMask == 0 && GetIdentifierSegment(r_idx, end_idx, type, MName, VName, FName, argbegin, end)) {
		...
		/*for (size_t i = 0; i < tlist.size(); ++i) {
			...
			switch (tlist[i].iType) {
			case 0:
				printf("SubExpr  : %zd Type: %d, Module Name: %s, Variable Name: %s, Member: %s, argbeg: %zd, end: %zd\n", tlist[i].idx, tlist[i].type, tlist[i].MName.c_str(), tlist[i].VName.c_str(), tlist[i].FName.c_str(), tlist[i].argbegin, tlist[i].end);
				break;
			case 1:
				printf("Int      : %zd  %lld\n", tlist[i].idx, tlist[i].iToken.GetValue<long long>());
				break;
			case 2:
				printf("FLOAT    : %zd  %f\n", tlist[i].idx, tlist[i].iToken.GetValue<float>());
				break;
			case 3:
				printf("STRING   : %zd  %s\n", tlist[i].idx, tlist[i].iToken.GetValue<std::string>().c_str());
				break;
			case 4:
				printf("SubStatement : %zd  to %zd\n", tlist[i].idx, tlist[i].end);
				break;
			case 5:
				printf("OPEARTOR : %zd  %s\n", tlist[i].idx, TypeDescString(tlist[i].iToken.m_Desc).c_str());
				break;
			}
		}*/
		...
		size_t list_idx = 0;
		StatementSegment prev = tlist[list_idx];
		list_idx++;

		bool hasAssignmentOperator = false;
		for (; list_idx < tlist.size(); ++list_idx) {
			if (tlist[list_idx].iType == -1) {
				m_ErrorMachine->AddError("Unknown error, type = error.", m_Tokens[tlist[list_idx].idx].m_ColNumber, m_Tokens[tlist[list_idx].idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
				return StatementSegment{};
			}

			if (tlist[list_idx].iType < 5) {
				if (prev.iType != 5) {
					m_ErrorMachine->AddError("Missing operator before identifier.", m_Tokens[tlist[list_idx].idx].m_ColNumber, m_Tokens[tlist[list_idx].idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
					return StatementSegment{};
				}
			}
			else if (tlist[list_idx].iType == 5) {
				if (tlist[list_idx].iToken.m_Desc == SlgRuleTable::e_SlgTokenDesc::SELFADD || tlist[list_idx].iToken.m_Desc == SlgRuleTable::e_SlgTokenDesc::SELFSUB) {
					if (list_idx != 1) {
						m_ErrorMachine->AddError("Self Add/Sub operator is only allowed in the second location.", m_Tokens[tlist[list_idx].idx].m_ColNumber, m_Tokens[tlist[list_idx].idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
						return StatementSegment{};
					}
					else if (!(prev.iType == 0 && (prev.type == 1 || prev.type == 2 || prev.type == 4 || prev.type == 5))) {
						m_ErrorMachine->AddError("Self Add/Sub operator can only be used with variables, not with subexpressions and functions.", m_Tokens[tlist[list_idx].idx].m_ColNumber, m_Tokens[tlist[list_idx].idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
						return StatementSegment{};
					}
				}
				else if (tlist[list_idx].iToken.IsUnaryOperator()) {
					if (prev.iType == 5 && prev.iToken.IsUnaryOperator()) {
						m_ErrorMachine->AddError("Can't using double unary operator.", m_Tokens[tlist[list_idx].idx].m_LineNumber, m_Tokens[tlist[list_idx].idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TYPE);
						return StatementSegment{};
						...
						if (list_idx != 2) {
							m_ErrorMachine->AddError("Missing operator before unary operator.", m_Tokens[tlist[list_idx].idx].m_LineNumber, m_Tokens[tlist[list_idx].idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TYPE);
							return StatementSegment{};
						}
					}
				}
				else if (tlist[list_idx].iToken.IsBinaryOperator()) {
					if (prev.iType < 5) {
						//correct
					}
					else {
						m_ErrorMachine->AddError("Missing number before binary operator.", m_Tokens[tlist[list_idx].idx].m_LineNumber, m_Tokens[tlist[list_idx].idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TYPE);
						return StatementSegment{};
					}
				}
				else if (tlist[list_idx].iToken.IsAssignmentOperator()) {
					if (recursionDepth != 0) {
						m_ErrorMachine->AddError("Assignment operator is not allowed in the subexpressions.", m_Tokens[tlist[list_idx].idx].m_ColNumber, m_Tokens[tlist[list_idx].idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
						return StatementSegment{};
					}
					if (list_idx != 1) {
						m_ErrorMachine->AddError("Assignment operator is only allowed in the second location.", m_Tokens[tlist[list_idx].idx].m_ColNumber, m_Tokens[tlist[list_idx].idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
						return StatementSegment{};
					}
					else if (!(prev.iType == 0 && (prev.type == 1 || prev.type == 2 || prev.type == 4 || prev.type == 5))) {
						m_ErrorMachine->AddError("Assignment operator can only be used with variables, not with subexpressions and functions.", m_Tokens[tlist[list_idx].idx].m_ColNumber, m_Tokens[tlist[list_idx].idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
						return StatementSegment{};
					}
					hasAssignmentOperator = true;
				}

				propList.push_back(tlist[list_idx].iToken.GetOperatorProperity());
			}
			else {
				m_ErrorMachine->AddError("Expressions unkown error.", m_Tokens[tlist[list_idx].idx].m_ColNumber, m_Tokens[tlist[list_idx].idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
				return StatementSegment{};
			}
			prev = tlist[list_idx];
		}
		...
		if (tlist.size() == 1) {
			return SingleStatementSegmentSolver(tlist[0]);
		}
		else if (tlist.size() == 2 && recursionDepth == 0) { //self sub add
			return tlist[0];
		}
		else if (hasAssignmentOperator) {
			//有赋值语句
			StatementSegment B = ExprSolverCall(minProp, maxProp, minProp, tlist, 2, tlist.size() - 1);
			StatementSegment A = VariableSolver(tlist[0]);
			if (!m_ErrorMachine->Success()) {
				return StatementSegment{};
			}
			StatementSegment res = SovlerAsmAssignmentOperator(tlist[1], A, B);
			if (!m_ErrorMachine->Success()) {
				return StatementSegment{};
			}
			else {
			...
			StatementSegment res = ExprSolverCall(minProp, maxProp, minProp, tlist, 0, tlist.size() - 1);
			if (!m_ErrorMachine->Success()) {
				return StatementSegment{};
			}
			else {
	...
	inline StatementSegment SingleStatementSegmentSolver(StatementSegment& item) {
		if (item.iType == 0) {
	...
	inline StatementSegment ExprSolverCall(size_t minprop, size_t maxprop, size_t currentprop, std::vector<StatementSegment>& items, size_t start, size_t end) {

		if (currentprop > maxprop + 1) { return StatementSegment{}; }
		...
		if (currentprop == 14 && items[start].iType == 5 && items[start].iToken.IsUnaryOperator()) { //一元运算符
			size_t next = start + 1;
			StatementSegment val = SingleStatementSegmentSolver(items[next]);
			return SovlerAsmUnaryOperator(items[start], val);
		}
		else {
			std::vector<StatementSegment> varlayer;
			std::vector<StatementSegment> oplayer;
			...
			for (size_t i = start; i <= end; i++) {
				if (items[i].iType == 5) {
					...
					if (items[i].iToken.GetOperatorProperity() == currentprop) {
						...
						StatementSegment newVar = ExprSolverCall(minprop, maxprop, currentprop + nearlestPropDelta, items, s_mask, i - 1);
						if (!m_ErrorMachine->Success()) {
							return StatementSegment{};
						}
			...
			StatementSegment newVar = ExprSolverCall(minprop, maxprop, currentprop + nearlestPropDelta, items, s_mask, end);
			if (!m_ErrorMachine->Success()) {
				return StatementSegment{};
			}
			varlayer.push_back(newVar);
			...
			//变量运算
			StatementSegment A{};
			if (varlayer[0].iType == 0 || varlayer[0].iType == 7) { //是变量 或者 是寄存器
				A = varlayer[0];
				A.varType = varlayer[0].varType;
			}
			else {
				int rehIdx = GetFreeRegister();

				A.iType = 7;
				A.Reg = rehIdx;
				A.varType = varlayer[0].varType;
				std::cout << "MOV " << DescGenerator(A).desc << ", " << DescGenerator(varlayer[0]).desc << "\n";
			}

			for (size_t i = 0; i < oplayer.size(); i++) {
				StatementSegment B = varlayer[i + 1];
	...
	inline StatementSegment FunctionSolver(StatementSegment& item) {
		//函数参数分离
		//检查部分
		...
		if (item.type == 7) { // 如果是普通函数
			if (item.MName != "") {  //如果是外部函数
				NeedCheck = false;
			}
			else {
				auto it = g_Config.StandardFunction.find(item.VName);
				if (it != g_Config.StandardFunction.end()) {
					retType = it->second.returnType;
					args = it->second.arguments;
				}
				else {
					m_ErrorMachine->AddError("Function: \"" + item.VName + "\" is not defined", m_Tokens[item.idx].m_LineNumber, m_Tokens[item.idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
					return StatementSegment{};
				}
			}
		}
			...
			if (item.type == 6) { // 如果是成员函数
				...
				if (it != g_Config.MemberFunction.end() && it->second.find(item.FName) != it->second.end()) {
					auto& func = it->second[item.FName];
					retType = func.returnType;
					args = func.arguments;
				}
				else {
					m_ErrorMachine->AddError("Local Variable: \"" + item.VName + " \",Type: \"" + item.varType + "\" doesn't have Member Function \"" + item.FName + "\" not defined", m_Tokens[item.idx].m_LineNumber, m_Tokens[item.idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
					return StatementSegment{};
				}
			}
			else if (item.type == 3) {
				...
				if (it != g_Config.MemberFunction.end() && it->second.find(item.FName) != it->second.end()) {
					...
					m_ErrorMachine->AddError("Attribute: \"" + item.VName + " \",Type: \"" + item.varType + "\" doesn't have Member Function \"" + item.FName + "\" not defined", m_Tokens[item.idx].m_LineNumber, m_Tokens[item.idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
					return StatementSegment{};
				}
			}
		}
		...
		if (NeedCheck) {
			if (cArgs.size() != args.size()) {
				if (item.type == 3 || item.type == 6) {
					m_ErrorMachine->AddError("Member Function: \"" + item.FName + " \" in Type: \"" + item.varType + "\" need the same number of arguments (need " + std::to_string(args.size()) + " arguments, but now have " + std::to_string(cArgs.size()) + ").", m_Tokens[item.idx].m_LineNumber, m_Tokens[item.idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
					return StatementSegment{};
				}
				else {
					m_ErrorMachine->AddError("Function: \"" + item.VName + "\" need the same number of arguments (need " + std::to_string(args.size()) + " arguments, but now have " + std::to_string(cArgs.size()) + ").", m_Tokens[item.idx].m_LineNumber, m_Tokens[item.idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
					return StatementSegment{};
				}
			}
		}
		...
		for (size_t i = 0; i < cArgs.size(); ++i) {
			StatementSegment temp = ExprSolver(cArgs[i].first, cArgs[i].second);
			...
			if (NeedCheck) {
				if (temp.varType != args[i]) {
					if (temp.varType == "float" && args[i] == "int") {
						if (temp.iType == 2) {
							temp.iType = 1;
							temp.iToken.m_Value = (long long)(temp.iToken.GetValue<float>());
						}
						temp.varType = "int";
					}
						...
						if (temp.iType == 1) {
							temp.iType = 2;
							temp.iToken.m_Value = (float)(temp.iToken.GetValue<long long>());
						}
						temp.varType = "float";
					}
					else {
						if (item.type == 3 || item.type == 6) {
							m_ErrorMachine->AddError("Member Function: \"" + item.FName + " \" in Type: \"" + item.varType + "\" argument " + std::to_string(i) + "," + args[i] + ")is not match with \"" + temp.varType + "\"", m_Tokens[cArgs[i].first].m_LineNumber, m_Tokens[cArgs[i].first].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
							return StatementSegment{};
						}
						else {
							m_ErrorMachine->AddError("Function: \"" + item.VName + "\" argument (" + std::to_string(i) + "," + args[i] + ") is not match with \"" + temp.varType + "\"", m_Tokens[cArgs[i].first].m_LineNumber, m_Tokens[cArgs[i].first].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
							return StatementSegment{};
						}
					}
				}
		...
		if (item.type == 3)
		{
			if (item.MName != "")
			{
				item.desc = item.MName + "::";
			}
			item.desc += item.VName + "." + item.FName;
			printf("CALLA %s\n", item.desc.c_str());
		}
		else if (item.type == 6)
		{
		...
		else if (item.type == 7)
		{
			...
			item.desc += item.VName;
			printf("CALL %s\n", item.desc.c_str());
		}

		StatementSegment vtemp;
		vtemp.iType = 7;
	...
	inline StatementSegment DescGenerator(StatementSegment& item) {

		if (item.iType == 1) {
			item.desc = std::to_string(item.iToken.GetValue<long long>());
		}
		else if (item.iType == 2) {
			item.desc = std::to_string(item.iToken.GetValue<float>());
		}
		else if (item.iType == 4) {
	...
	inline StatementSegment VariableSolver(StatementSegment& item) {

		if (item.type == 1 || item.type == 2) {
			if (item.MName == "") { //如果是内部函数
				if (m_ProgramTable.m_Attribute.find(item.VName) == m_ProgramTable.m_Attribute.end()) {
					m_ErrorMachine->AddError("Attribute \"" + item.VName + "\" not defined", m_Tokens[item.idx].m_LineNumber, m_Tokens[item.idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
					return StatementSegment{};
				}
				if (item.type == 2) {
					if (item.type == 2) {
						auto mFind = g_Config.MemberVariable.find(item.varType);
						if (mFind == g_Config.MemberVariable.end() || mFind->second.find(item.FName) == mFind->second.end()) {
							m_ErrorMachine->AddError("Attribute: \"" + item.VName + " \",Type: \"" + item.varType + "\" doesn't have Member Variable \"" + item.FName + "\" not defined", m_Tokens[item.idx].m_LineNumber, m_Tokens[item.idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
							return StatementSegment{};
						}
					}
				}
			...
			if (m_ProgramTable.m_Attribute.find(item.VName) == m_ProgramTable.m_Attribute.end()) {
				m_ErrorMachine->AddError("Local varaible \"" + item.VName + "\" not defined", m_Tokens[item.idx].m_LineNumber, m_Tokens[item.idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
				return StatementSegment{};
			}
			...
			if (item.type == 5) {
				auto mFind = g_Config.MemberVariable.find(item.varType);
				if (mFind == g_Config.MemberVariable.end() || mFind->second.find(item.FName) == mFind->second.end()) {
					m_ErrorMachine->AddError("Local varaible: \"" + item.VName + " \",Type: \"" + item.varType + "\" doesn't have Member Variable \"" + item.FName + "\" not defined", m_Tokens[item.idx].m_LineNumber, m_Tokens[item.idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TOKEN);
					return StatementSegment{};
				}
			}
		}
	...
	inline StatementSegment ConstSolver(StatementSegment& item) {

		return DescGenerator(item);
	}

	inline StatementSegment SovlerAsmUnaryOperator(StatementSegment& op, StatementSegment& A) {

		StatementSegment reg{};
		reg.varType = A.varType;
		if (A.iType == 1) {
			reg = A;
			reg.iToken.m_Value = IntConstexprCalcValueUnary(op.iToken.m_Desc, A.iToken.GetValue<long long>());
			DescGenerator(reg);
			return reg;
		}
		if (A.iType != 7) {
	...
	inline StatementSegment SovlerAsmBinaryOperator(StatementSegment& op, StatementSegment& A, StatementSegment& B) {
		//检查部分
		...
		if (A.iType == 1 && B.iType == 1) {
			reg = A;
			reg.iToken.m_Value = IntConstexprCalcValueBinary(op.iToken.m_Desc, A.iToken.GetValue<long long>(), B.iToken.GetValue<long long>());
			DescGenerator(reg);
			return reg;
		}
		if (A.iType != 7) {
	...
	inline StatementSegment SovlerAsmAssignmentOperator(StatementSegment& op, StatementSegment& A, StatementSegment& B) {
		switch (op.iToken.m_Desc) {
	...
	inline long long IntConstantExprSolver(size_t& idx, SlgRuleTable::e_SlgTokenDesc endSign, std::string errorMsg = "") {  // token指针在返回时，指向endsign的位置

		if (m_Tokens[idx + 1].m_Desc == endSign) {
			if (m_Tokens[idx].IsNumber()) {
				idx++;
				if (m_Tokens[idx].m_Desc == SlgRuleTable::e_SlgTokenDesc::FLOAT) {
					return static_cast<long long>(m_Tokens[idx - 1].GetValue<float>());
				}
				return m_Tokens[idx - 1].GetValue<long long>();
			}
			else {
				m_ErrorMachine->AddError("Illegal identifier.", m_Tokens[idx].m_LineNumber, m_Tokens[idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TYPE);
				return 0;
			}
		}
		...
		SlgTokenItem prev = m_Tokens[curr_idx];

		if (!(prev.IsNumber() || prev.m_Desc == SlgRuleTable::e_SlgTokenDesc::BRACKET_ROUND_START)) {
			m_ErrorMachine->AddError("opeartion statement only start by number or round bracket.", m_Tokens[curr_idx].m_LineNumber, m_Tokens[curr_idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TYPE);
			return 0;
		}
		if (prev.m_Desc == SlgRuleTable::e_SlgTokenDesc::BRACKET_ROUND_START) {
			bracketMask++;
		}
		if (prev.IsNumber()) {
			numCount++;
		...
		for (;; curr_idx++) {
			...
			if (m_Tokens[curr_idx].IsBinaryOperator()) {
				if (!(prev.IsNumber() || prev.m_Desc == SlgRuleTable::e_SlgTokenDesc::BRACKET_ROUND_END)) {
					m_ErrorMachine->AddError("Missing operator before number.", m_Tokens[curr_idx].m_LineNumber, m_Tokens[curr_idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TYPE);
					return 0;
				}
				minProp = std::min(minProp, m_Tokens[curr_idx].GetOperatorProperity());
				maxProp = std::max(maxProp, m_Tokens[curr_idx].GetOperatorProperity());
				binaryOpCount++;
			}
			else if (m_Tokens[curr_idx].IsUnaryOperator()) {
				if (prev.IsUnaryOperator()) {
					m_ErrorMachine->AddError("Can't using double unary operator.", m_Tokens[curr_idx].m_LineNumber, m_Tokens[curr_idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TYPE);
					return 0;
				}
				if (!(prev.IsBinaryOperator() || prev.m_Desc == SlgRuleTable::e_SlgTokenDesc::BRACKET_ROUND_START)) {
					m_ErrorMachine->AddError("Missing operator before unary operator.", m_Tokens[curr_idx].m_LineNumber, m_Tokens[curr_idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TYPE);
					return 0;
				}
				maxProp = std::max(maxProp, m_Tokens[curr_idx].GetOperatorProperity());
			}
			else if (m_Tokens[curr_idx].IsNumber()) {
				if (!(prev.IsBinaryOperator() || prev.IsUnaryOperator() || prev.m_Desc == SlgRuleTable::e_SlgTokenDesc::BRACKET_ROUND_START)) {
					m_ErrorMachine->AddError("Missing operator before number.", m_Tokens[curr_idx].m_LineNumber, m_Tokens[curr_idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TYPE);
					return 0;
				}
				numCount++;
			}
			else if (m_Tokens[curr_idx].m_Desc == SlgRuleTable::e_SlgTokenDesc::BRACKET_ROUND_START) {
				if (!(prev.IsBinaryOperator() || prev.IsUnaryOperator()) || prev.m_Desc == SlgRuleTable::e_SlgTokenDesc::BRACKET_ROUND_START) {
					m_ErrorMachine->AddError("Missing operator before start round bracket.", m_Tokens[curr_idx].m_LineNumber, m_Tokens[curr_idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TYPE);
					return 0;
				}
				bracketMask++;
			}
			else if (m_Tokens[curr_idx].m_Desc == SlgRuleTable::e_SlgTokenDesc::BRACKET_ROUND_END) {
				if (!(prev.IsNumber() || prev.m_Desc == SlgRuleTable::e_SlgTokenDesc::BRACKET_ROUND_END)) {
					m_ErrorMachine->AddError("Missing number before end round bracket.", m_Tokens[curr_idx].m_LineNumber, m_Tokens[curr_idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TYPE);
					return 0;
				}
				bracketMask--;
			}
			else if (m_Tokens[curr_idx].m_Desc == endSign) {
				if (bracketMask != 0) {
					m_ErrorMachine->AddError("Parentheses do not match.", m_Tokens[curr_idx].m_LineNumber, m_Tokens[curr_idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TYPE);
					return 0;
				}

				if (binaryOpCount + 1 != numCount) {
					m_ErrorMachine->AddError("The number of operators and operands do not match.", m_Tokens[start_idx].m_LineNumber, m_Tokens[start_idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_OPERATOR_CONTENT);
					return false;
				}
				...
				if (errorMsg == "") {
					m_ErrorMachine->AddError("Illegal identifier appears in  operation statement.", m_Tokens[curr_idx].m_LineNumber, m_Tokens[curr_idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TYPE);
				}
				else {
					m_ErrorMachine->AddError(errorMsg, m_Tokens[curr_idx].m_LineNumber, m_Tokens[curr_idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_UNEXPECTED_TYPE);
				}

				return 0;
			}
			prev = m_Tokens[curr_idx];
		}

		return IntConstantExprCalcCall(minProp, maxProp, minProp, start_idx, end_idx);
	}

	inline long long IntConstantExprCalcCall(size_t minprop, size_t maxprop, size_t currentprop, size_t start_idx, size_t end_idx) {

		if (currentprop > maxprop + 1) { return 0; } // 防止无意义递归
		if (start_idx > end_idx) {
			m_ErrorMachine->AddError("Operation statement unknown error.", m_Tokens[start_idx].m_LineNumber, m_Tokens[start_idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_OPERATOR_CONTENT);
			return 0;
		}
		...
		if (end_idx == start_idx) {
			...
			if (m_Tokens[start_idx].IsNumber()) {
				if (m_Tokens[start_idx].m_Desc == SlgRuleTable::e_SlgTokenDesc::FLOAT) {
					return static_cast<long long>(m_Tokens[end_idx].GetValue<float>());
				}
				return m_Tokens[end_idx].GetValue<long long>();
			}
			else {
				m_ErrorMachine->AddError("The minimum token is not a number.", m_Tokens[start_idx].m_LineNumber, m_Tokens[start_idx].m_ColNumber, SlgRuleTable::e_SlgErrorNumber::ERROR_OPERATOR_CONTENT);
				return 0;
			}
		}

		if (currentprop == 14 && m_Tokens[start_idx].IsUnaryOperator()) { //一元运算符
			size_t idx = start_idx;
			if (m_Tokens[idx + 1].IsNumber()) {
				long long varb = 0;
				if (m_Tokens[idx + 1].m_Desc == SlgRuleTable::e_SlgTokenDesc::FLOAT) {
					varb = static_cast<long long>(m_Tokens[idx + 1].GetValue<float>());
				}
				varb = static_cast<long long>(m_Tokens[idx + 1].GetValue<long long>());
				return IntConstexprCalcValueUnary(m_Tokens[start_idx].m_Desc, varb);
			}
			else {
				long long varb = IntConstantExprCalcCall(minprop, maxprop, minprop, start_idx + 1, end_idx);
				return IntConstexprCalcValueUnary(m_Tokens[start_idx].m_Desc, varb);
			}
		}
		else {
			std::vector<SlgRuleTable::e_SlgTokenDesc> currentLayerOperator;
			std::vector<long long> currentLayerValue;
			...
			while (idx <= end_idx) {
				...
				else if (bracketMask == 0) {

					if (m_Tokens[idx].IsOperator()) {
						int propDelta = m_Tokens[idx].GetOperatorProperity() - currentprop;
						if (propDelta > 0) {
							nearlestPropDelta = std::min(nearlestPropDelta, propDelta);
						}
					}

					if (m_Tokens[idx].IsBinaryOperator()) {

						if (m_Tokens[idx].GetOperatorProperity() == currentprop) {
							operNum++;

							long long newVar = IntConstantExprCalcCall(minprop, maxprop, currentprop + 1, start, idx - 1);
							currentLayerValue.push_back(newVar);
							currentLayerOperator.push_back(m_Tokens[idx].m_Desc);
							start = idx + 1;
						}

					}
					else if (m_Tokens[idx].IsUnaryOperator()) {
			...
			//如果没有检测到任何运算符， 要么是在当前运算表达式中没有当前优先级的运算符存在， 要么是括号问题
			if (operNum == 0) {
				if (nearlestPropDelta != 999) { // 如果不是被括号包裹着
					return IntConstantExprCalcCall(minprop, maxprop, currentprop + nearlestPropDelta, start_idx, end_idx);
				}
				else {  //全处于括号之中
					return IntConstantExprCalcCall(minprop, maxprop, minprop, start_idx + 1, end_idx - 1);
				}
			}
			else {
				long long newVar = IntConstantExprCalcCall(minprop, maxprop, currentprop + 1, start, idx - 1);
			...
			long long  res = currentLayerValue[0];
			for (int i = 0; i < currentLayerOperator.size(); i++) {
				int B = currentLayerValue[i + 1];
				res = IntConstexprCalcValueBinary(currentLayerOperator[i], res, B);
			}
	...
	inline long long IntConstexprCalcValueUnary(SlgRuleTable::e_SlgTokenDesc desc, long long value) {
		switch (desc) {
		case SlgRuleTable::e_SlgTokenDesc::NOT:
			return value == 0 ? 1 : 0;
		case SlgRuleTable::e_SlgTokenDesc::INVERT:
	...
	inline long long IntConstexprCalcValueBinary(SlgRuleTable::e_SlgTokenDesc desc, long long A, long long B) {
		switch (desc) {
		case SlgRuleTable::e_SlgTokenDesc::ADD:
			return A + B;
		case SlgRuleTable::e_SlgTokenDesc::SUB:
